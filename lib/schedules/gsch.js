...
function newProject (month, callback) {
  tasksModel.find({ type: "general_sch", month: month, person: /_Project/ }, function (err, result) {
    if (err) { return callback(err, null) }
...
// var newPath = new tasksModel({ type: "general_sch", month: 201807, day: 3, person: "JBB", details: "OFF", details_color: "", project: "_0", background: "mesh" })
// // Record on MongoDB !
...
function genMonth (obj_t, date, month, template, project, person, callback) {
  return new Promise(function (resolve, reject) {
    ...
    // Fill tasks object
    tasksModel.find({ type: "general_sch", month: month }, function(err, results) {
      if (err) { reject(err) }
...
function project_used (month, pos, persons, project, callback) {
  if (findHeader(persons).indexOf('_Project') > -1) {
    var query = { type: "general_sch", month: month, day: { $gte: pos[0], $lte: pos[1] }, person: /_Project/ }
    ...
    project_used (month, pos, persons, project[0], function (err, persons) {
      for (var person in persons) {
        for (var i = pos[0]; i <= pos[1]; i++) {
          var query = { type: "general_sch", month: month, day: i, person: person }
...
gsch.del_prj = function (task, callback) {
  ...
  projectModel.findOneAndDelete({ shortId: task.shortId }, function(err, project) {
    ...
    tasksModel.find({ type: "general_sch", project: task.shortId }, function(err, months) {
      if (err) { return callback(err, null) }

      months.forEach(function(res) {
        var query = { type: "general_sch", month: res.month, day: res.day, person: res.person, project: task.shortId }
  ...
  infoModel.findOne({}, function(err, info) {
    ...
    infoModel.update({}, { team: team }, function (err) {
      ...
      scheduleModel.find({ month: { $gte: task.monthFrom }}, function(err, result) {
        ...
        tasksModel.find({ type: "general_sch", month: { $gte: task.monthFrom }, person: task.person}, function(err, result) {
          if (err) { return callback(err, null) }
